...
class MemberController extends BaseController
{
    ...
    public function index(Request $request){
        ...
        //查询数据
        $data = Member::orderBy('id','desc')->paginate($data['per_page'],$columns = ['*'], $pageName = '',$data['page']);
        //展示视图
        return view('dcat.member.index',compact('data'));
    ...
    public function add(){
        ...
        if(Input::method() == 'POST'){
            ...
            if($result){
                return show::error('用户已存在！');
            }else{
                // $data['avatar'] = "/images/th.jpg";
                $result = Member::insert($data);
                return $result ? show::success('ok'):show::error('添加失败');
            }

        }else{
            //展示视图
            return view('dcat.member.add');
        }
    }
    //批量添加会员
    ...
    public function info()
    {
        ...
        return view('dcat.member.info',compact('user'));
    }
    ...
    public function initInfo(Request $request)
    {
        ...
        if($request->method() == 'POST'){
            ...
            $user->isInit = '1';
            return $user->save() ? show::success('ok') : show::error('error');
        }else{

            return view('dcat.member.initInfo',compact('user'));
        }
    }
    ...
    public function modify($id){
        ...
        if(Input::method() == 'POST'){
            ...
            return view('dcat.member.modify',compact('data'));
        }
    }
    ...
    public function report($id){
        ...
        if(Input::method() == 'POST'){
            ...
            return view('dcat.report.index',compact('project_group'));
        }
    }
    ...
    public function memberexcel($id){
        if(Input::method() == 'POST'){
            ...
            $result = $user->save();
            return $result ? show::success('OK') : show::error('修改失败！');
        }else{
            $user = Member::where('id',$id)->get()->first();
            return view('dcat.member.excel',compact('user'));
        }
    }
    ...
    public function apply(Request $request)
    {
        if($request->method() == 'GET'){
            ...
            return view('dcat.member.apply',compact('res'));
    ...
    public function applyIndex(Request $request)
    {
        ...
        $data = ApplyToMember::paginate($data['per_page'],$columns = ['*'], $pageName = '',$data['page']);
        foreach ($data as $k => &$v){
        ...
        return view('dcat.member.applyAuth',compact('data'));
    }
