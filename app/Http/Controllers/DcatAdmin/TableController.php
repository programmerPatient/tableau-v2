...
class TableController extends Controller
{
    public function index(Request $request){
        ...
        return view('dcat.table.index',compact('site_root','parentId','contentUrl','ticket','filter','toolbar','system','report_id','hascontentUrl'));
    }
    ...
    public function status(){
        ...
        $result->status = $type;
        $res = $result->save();
        return $res?show::success('OK'):show::error('修改失败');
    }
...
//                $hasTableauIds = explode(',',$user->tableauIds);
                return view('dcat.table.authIndex',compact('p','hasTableauIds'));//展示报表列表
            }
        }
    }
    //批量报表权限的分配
    public function auths($ids){
        ...
        if(Input::method() == 'POST'){
            ...
            if($data === false){
                ...
                return view('dcat.table.authsIndex',compact('p'));//展示报表列表
            }
        }
    }
    ...
    public function user($id){
        ...
        if(Input::method() == 'POST'){
            ...
            return view('dcat.table.user',compact('tsResponse','mamber'));
        }
    }
    ...
    public function users($ids){
        ...
        if(Input::method() == 'POST'){
            ...
            return view('dcat.table.users',compact('tsResponse'));
        }
    }
