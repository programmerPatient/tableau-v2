...
class PublicController extends BaseController
{
    ...
    public function login(Request $request){
        ...
        if(!$result){
            return view('dcat.inization.index');//初始化基本信息
        }
        $system = System::get()->first();
        if(!$system){
            return view('dcat.inization.system');
        }
        $sy = SmsConfig::get()->first();
        if(!$sy){
            return view('dcat.inization.smsInit');
        }
        $notAuth = $request->notAuth;
        $tableau_domain = $system->system_domain;
        Session::put(['tableau_domain' => $tableau_domain]);
        return view('dcat.public.login',compact('system','notAuth'));
    }
    ...
    {
        $system = System::get()->first();
        return view('dcat.public.sms',compact('system'));
    }
    ...
    {
        return view('dcat.public.register');
